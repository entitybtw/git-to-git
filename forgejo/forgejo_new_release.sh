#!/bin/bash
# Sync ONLY latest GitHub release to Forgejo (no cleanup)
...
# Check if already exists on Forgejo
forgejo_release=$(curl -s -H "Authorization: token $FORGEJO_TOKEN" \
  "$FORGEJO_URL/api/v1/repos/$FORGEJO_REPO/releases/tags/$tag")

if echo "$forgejo_release" | jq -e '.id // empty' >/dev/null 2>&1; then
...
echo "$latest_release" | jq -c '.assets[] // empty' | while IFS= read -r asset_json; do
  ...
  curl -s -X POST "$FORGEJO_URL/api/v1/repos/$FORGEJO_REPO/releases/$release_id/assets?name=$asset_name" \
    -H "Authorization: token $FORGEJO_TOKEN" \
...
echo "Latest release $tag synced to Forgejo!"
